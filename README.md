...
La comunicación usando LoRa también es una comunicación serie que utiliza el protocolo UART. Los programas de los Arduinos deben crear un canal de comunicación usando la librería _SoftwareSerial_ exactamente igual que hasta ahora. Por tanto, el uso de LoRa no implica ningún cambio en el código de los programas. Eso sí, es necesario incorporar los elementos hardware del kit, que es lo que resumimos a continuación.   
...
La solución al nivel de protocolo de aplicación implica añadir a cada mensaje que se envía una cabecera con un identificador (por ejemplo, el número de grupo de proyecto) de manera que el programa que recibe el mensaje solo hace caso de los mensajes que llevan la cabecera adecuada. Esto resuelve el problema pero no es una solución muy eficiente porque consume tiempo de procesado.   

La solución a nivel de protocolo de transporte es hacer que el propio chip de comunicacion LoRa añada la cabecera a los mensajes, de forma transparence a la aplicación de manera que el chip receptor solo entregará a la aplicacion los mensajes con la cabecera adecuada. La solución es similar a la anterior pero con la diferencia de que la cabecera la añade el chip por hardware, con lo cual no se consume tiempo de procesado, aunque el chip sigue tenienendo trabajo para seleccionar entre todos los mensajes que recibe los que son para él.   
